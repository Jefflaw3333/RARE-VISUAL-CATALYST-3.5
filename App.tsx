    ...
    <div className="mb-2">
        ...
        >
            <h4 className="text-sm font-medium text-slate-400 group-hover:text-slate-200 transition-colors flex items-center gap-2">
                {title}
                {!isOpen && <span className="text-xs text-slate-600 font-normal opacity-0 group-hover:opacity-100 transition-opacity">(Show)</span>}
            </h4>
            <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 text-slate-500 transition-transform duration-200 ${isOpen ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
            </svg>
        </button>
        {isOpen && (
            <div className="pb-2 animate-fade-in-fast">
                {children}
            </div>
        )}
    </div>
...
const App: React.FC = () => {
  ...
  const currentTotalOutputs = effectiveAngleCount * imagesPerAngle;
    ...
    const handleAngleTagChange = (angleId: string) => {
        ...
        if (isSelected) {
            // Deselecting
            const remaining = totalSelectedCount - 1;
            newEffectiveCount = remaining === 0 ? 1 : remaining;
        } else {
            // Selecting
            const newCount = totalSelectedCount + 1;
            newEffectiveCount = newCount;
        }
    ...
    const handleAddCustomAngle = () => {
        ...
        if (newAngle && !customAngles.includes(newAngle)) {
            const newCount = totalSelectedCount + 1;
            const newTotalOutputs = newCount * imagesPerAngle;
